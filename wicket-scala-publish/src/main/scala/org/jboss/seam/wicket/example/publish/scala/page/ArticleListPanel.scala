 ...
 * @author oranheim
 */
class ArticleListPanel extends Panel("articlePanel") {

    add(new Link("create-link") {
        def onClick() = onCreateArticle
    })
    
    add(new ArticleListView(this, Generics.model(new Model[ArrayList[Article]]())))
    
    setOutputMarkupId(true)

    def onCreateArticle = {
        var editPanel = new ArticleEditPanel
        editPanel.getArticleHome().setId(null)
        editPanel.getArticleHome().setInstance(null)
